...
if (isset($_POST['edit'])) {
    ...
    $user_id = $_SESSION['id'];
    $foto = $_FILES['lokasi_file']['name'];
    // var_dump($_FILES['lokasi_file]['name]);
    $tmp = $_FILES['lokasi_file']['tmp_name'];
    $lokasi ='../assets/img/';
    $namafoto = rand().'-'.$foto.$temp;
    // var_dump($album_id);
    if (!isset($foto)) {
        $sql = mysqli_query($koneksi, "UPDATE foto SET judul_foto='$judul_foto',
        deskripsi_foto='$deskripsi_foto', tanggal_unduh='$tanggal_unduh', album_id=$album_id WHERE id=$id");
        var_dump($koneksi);
    }
     ...
     move_uploaded_file($tmp, $lokasi.$namafoto);
     $sql =mysqli_query($koneksi, "UPDATE foto SET judul_foto='$judul_foto',
     deskripsi_foto='$deskripsi_foto', tanggal_unduh='$tanggal_unduh', lokasi_file='$namafoto', album_id=$album_id WHERE id=$id");
     // var_dump($sql);
